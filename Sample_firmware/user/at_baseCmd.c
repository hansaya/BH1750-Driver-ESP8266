...
static volatile os_timer_t timer_TempSensor;
...
void polling_timer_elapsed(void *arg)
{
	...
	if(!GPIO_INPUT_GET(2)){
		...
		// int quality = 255;
  // 		int i;
		// int8_t ws_buffer[16*3];
		// for( i = 0; i < sizeof( ws_buffer ); i++ )
		// {
		// 	ws_buffer[i] = quality;
		// }
		// WS2812OutBuffer( ws_buffer, sizeof(ws_buffer) );
		// int error = bbb_dht_read(1, 14, &humid, &temper);
		// if(error ==1 )
		// 	os_sprintf(temp,"Temperature: %d and humid: %d >> error: %d\r\n",temper,humid, error);
		// else
		// 	os_sprintf(temp,"Something went wrong with temperature sensor!!\r\n");
		// uart0_sendStr(temp);
	}
